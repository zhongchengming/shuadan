...
Ajax.prototype.isError = function(data) {
	return data.resultCode == this.FAILED;
};
//判断请求是否成功
Ajax.prototype.isSuccess = function(data) {
	return data.resultCode == this.SUCCED;
};
//判断请求是否未查询到数据
Ajax.prototype.isNoResult = function(data) {
	return data.resultCode == this.No_RESULT;
};
//判断请求是否登录超时
Ajax.prototype.isTimeout = function(data) {
	return data.resultCode == this.LOGIN_OUT_TIME;
};
//判断请求是否权限不足
Ajax.prototype.authDenied = function(data) {
	return data.resultCode == this.FORBIDDEN;
};